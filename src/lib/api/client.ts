...
const ADMIN_API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api/v1'
...
adminClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    if (error.response?.status === 401 && originalRequest) {
      ...
      try {
        const response = await axios.post(`${ADMIN_API_BASE_URL}/admin/auth/refresh`, { refreshToken })
